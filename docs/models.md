...
2. Модели для произвольных таблиц, работающие через `illuminate/database` в целом и `Eloquent` в частности.

## Установка

1. Регистрируем пакет в `init.php` - `Uru\BitrixModels\ServiceProvider::register();`
...
```php
Uru\BitrixModels\Models\ElementModel;
Uru\BitrixModels\Models\SectionModel;
Uru\BitrixModels\Models\UserModel;
Uru\BitrixModels\Models\D7Model;
```
...
За подробностями смотрите  `vendor/uru-ruru/bitrix-mvc/src/Models/D7Model.php` и  `vendor/uru-ruru/bitrix-mvc/src/Queries/D7Query.php`
...
2. Добавляем в D7Model/EloquentModel трейт `use \Uru\BitrixModels\Models\Traits\DeactivationTrait;`
